
import javax.servlet.http.*;
import javax.servlet.*;
import java.io.*;
import java.net.*;
import java.util.*;

public class SaveRedlinesServlet extends HttpServlet {


public final void doGet(HttpServletRequest request, HttpServletResponse response)
throws ServletException, IOException
{

		PrintWriter outPut = response.getWriter();
		String option = request.getParameter("option");
		outPut.println("<html>");
		outPut.println("call by doGet SaveRedlinesServlet");
		outPut.println("</html>");
		outPut.flush();
		outPut.close();

	//System.out.println("AppendServlet: first in doGet");
}

public final void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String fileName = request.getParameter("file");
        String fileChunk = request.getParameter("file_content");
		String customContent = request.getParameter("custom_content");
		PrintWriter outPut = response.getWriter();

		File fdf = null;
		String txtSource = "";
		byte[] bArr = null;
		FileOutputStream fileOut = null;
		String str;

		/*****        DEBUG

		String current = new java.io.File( "." ).getCanonicalPath();

		// test
		fdf = File.createTempFile("SaveServlet-log",".txt");
		fileOut = new FileOutputStream( fdf );
		str= fdf.getPath();
		txtSource = "Save Servlet     "+current+"  "+fileName +"  "+fileChunk+"  \n";
		bArr = txtSource.getBytes();
		fileOut.write(bArr);
		fileOut.close();
		// I have to hardcode my current directory as I cannot get Java to understand where I am
		//String mycurrentDir = "C:\\xampp\\tomcat\\webapps\\CV-JS_2_3\\WEB-INF\\classes\\";

		DEBUG  *****/


		// open and write to file
		try
		{
			
			FileWriter fw = new FileWriter(decode(fileName),false);  //the save will create a new file
			fw.write(fileChunk); //appends the string to the file
			fw.close();

			outPut.println("Succes");


			fdf = File.createTempFile("SaveRedlineServlet-log-err",".txt");
			fileOut = new FileOutputStream( fdf );
			bArr = fileChunk.getBytes();
			fileOut.write(bArr);
			bArr = URLDecoder.decode( fileChunk, "UTF-8" ).getBytes();
			fileOut.write(bArr);
			fileOut.close();

/*
			saveFile(decode(fileName)+"xx1", fileChunk);
			fileChunk = URLDecoder.decode( fileChunk, "UTF-8" );
			saveFile(decode(fileName)+"xx2", fileChunk);
*/					
								
		}
		catch(IOException ioe)
		{

			fdf = File.createTempFile("SaveRedlineServlet-log-err",".txt");
			fileOut = new FileOutputStream( fdf );
			str= fdf.getPath();
			txtSource = "Save Servlet "+ioe+"  \n";
			bArr = txtSource.getBytes();
			fileOut.write(bArr);
			fileOut.close();
		}


}



public static String decode(String url){  

	try {  

		 String prevURL="";  

		 String decodeURL=url;  

		 while(!prevURL.equals(decodeURL))  

		 {  

			  prevURL=decodeURL;  

			  decodeURL=URLDecoder.decode( decodeURL, "UTF-8" );  

		 }  

		 return decodeURL;  

	} catch (UnsupportedEncodingException e) {  

		 return "Issue while decoding" +e.getMessage();  

	}  

}


public static String saveFile(String filename, String filecontent)
{
    String content = null;
    File file = new File(filename); //for ex foo.txt

    FileWriter writer = null;
    try {
        writer = new FileWriter(file);
		
        char[] ch = new char[filecontent.length()];
  
        // Copy character by character into array
        for (int i = 0; i < filecontent.length(); i++) {
            ch[i] = filecontent.charAt(i);
        }
        writer.write(ch);
        writer.close();
		
    }catch (Exception e) {
       // return(e.printStackTrace());
        return(e.toString());
    }
    return "Succes";
}




}